...
const inventorCheck = (inventor) => {
    return inventor.year > 1500 && inventor.year < 1599;
}

const inventorsArr = inventors.filter(inventorCheck);

veryOldInventors.push(inventorsArr);

console.log(inventorsArr, "a");
...
const invNames = inventors.map(inventor => `${ inventor.first} ${inventor.last}`);

inventorNames.push(invNames);
...
sortedByBirthYear.push(birthYear);
...
const joinName = people.map(name => {
    let [last, first] = name.split(',').map(part => part.trim());
    return `${first} ${last}`;
})
firstLast = joinName;
...
const rYou18 = devs.some(element => element.year < 2006);
isAdultPresent = rYou18
...
const devAge = devs.every(element => element.year < 2006);
isEveryone19OrOlder = devAge;
...
const commentCheck = comments.find(element => element.id === 823423);

commentById = commentCheck;
...
const commentIdxCheck = comments.findIndex((element, index) => {
